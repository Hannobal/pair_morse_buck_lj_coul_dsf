...
int ** morse_buck_ljd_gpu_compute_n(const int ago, const int inum,
                         ...
                         double *prd);
void morse_buck_ljd_gpu_compute(const int ago, const int inum,
                     const int nall, double **host_x, int *host_type,
...
PairMorseBuckLJCoulDSFGPU::PairMorseBuckLJCoulDSFGPU(LAMMPS *lmp) : PairMorseBuckLJCoulDSF(lmp), gpu_mode(GPU_FORCE)
{
  respa_enable = 0;
  reinitflag = 0;
  cpu_time = 0.0;
...
PairMorseBuckLJCoulDSFGPU::~PairMorseBuckLJCoulDSFGPU()
{
...
void PairMorseBuckLJCoulDSFGPU::compute(int eflag, int vflag)
{
  ...
  if (gpu_mode != GPU_FORCE) {
    inum = atom->nlocal;
    firstneigh = morse_buck_ljd_gpu_compute_n(neighbor->ago, inum, nall, atom->x,
    ...
    morse_buck_ljd_gpu_compute(neighbor->ago, inum, nall, atom->x, atom->type,
...
void PairMorseBuckLJCoulDSFGPU::init_style()
{
  if (!atom->q_flag)
  ...
  int success = morse_buck_ljd_gpu_init(atom->ntypes+1, cutsq, lj1, lj2, lj3, lj4,
                             offset, force->special_lj, atom->nlocal,
                             atom->nlocal+atom->nghost, 300, maxspecial,
                             cell_size, gpu_mode, screen, cut_ljsq, cut_coulsq,
                             force->special_coul, force->qqrd2e, e_shift,
                             f_shift, alpha, morse1, r0, beta, d0,
                             rhoinv, buck1, buck2, a, c);
...
double PairMorseBuckLJCoulDSFGPU::memory_usage()
{
  double bytes = Pair::memory_usage();
  return bytes + morse_buck_ljd_gpu_bytes();
}

/* ---------------------------------------------------------------------- */

void PairMorseBuckLJCoulDSFGPU::cpu_compute(int start, int inum, int eflag, int vflag,
                                      ...
                                      int **firstneigh)
{
  ...
  double prefactor,erfcc,erfcd,t,dexp,dr;
  int *jlist;
