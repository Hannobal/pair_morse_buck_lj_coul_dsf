...
PairMorseBuckLJCoulDSF::PairMorseBuckLJCoulDSF(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
}
...
PairMorseBuckLJCoulDSF::~PairMorseBuckLJCoulDSF()
{
  if (!copymode) {
    if (allocated) {
...
void PairMorseBuckLJCoulDSF::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,fpair;
  double r,rsq,r2inv,r6inv,forcecoul,forcelj,factor_coul,factor_lj;
  double prefactor,erfcc,erfcd,t,dr,dexp,rexp,forcebuck,forcemorse;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        fpair = (forcecoul + factor_lj*(forcelj+forcebuck)) * r2inv + forcemorse;
        f[i][0] += delx*fpair;
        f[i][1] += dely*fpair;
        f[i][2] += delz*fpair;
...
void PairMorseBuckLJCoulDSF::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairMorseBuckLJCoulDSF::settings(int narg, char **arg)
{
...
void PairMorseBuckLJCoulDSF::coeff(int narg, char **arg)
{
  if (narg < 10 || narg > 11)
  ...
  double a_one = force->numeric(FLERR,arg[5]);
  double rho_one = force->numeric(FLERR,arg[6]);
  if (rho_one <= 0) error->all(FLERR,"rho <= 0 for pair coefficients");
  double c_one = force->numeric(FLERR,arg[7]);
  double epsilon_one = force->numeric(FLERR,arg[8]);
  double sigma_one = force->numeric(FLERR,arg[9]);

  double cut_lj_one = cut_lj_global;
  if (narg == 11) cut_lj_one = force->numeric(FLERR,arg[10]);
...
void PairMorseBuckLJCoulDSF::init_style()
{
  if (!atom->q_flag)
...
double PairMorseBuckLJCoulDSF::init_one(int i, int j)
{
  ...
  if (offset_flag) {
    ...
    offset[i][j] = 4.0 * epsilon[i][j] * (pow(ratio,12.0) - pow(ratio,6.0))
                   ...
                   +a[i][j]*rexp - c[i][j]/pow(cut_lj[i][j],6.0);
  } else offset[i][j] = 0.0;
...
void PairMorseBuckLJCoulDSF::write_restart(FILE *fp)
{
...
void PairMorseBuckLJCoulDSF::read_restart(FILE *fp)
{
...
void PairMorseBuckLJCoulDSF::write_restart_settings(FILE *fp)
{
...
void PairMorseBuckLJCoulDSF::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairMorseBuckLJCoulDSF::single(int i, int j, int itype, int jtype, double rsq,
                                double factor_coul, double factor_lj,
                                double &fforce)
{
  double r2inv,r6inv,r,erfcc,erfcd,prefactor;
  double forcecoul,forcelj,dr,dexp,phicoul,philj;
  double rexp,forcebuck,phibuck;
  ...
  double eng = 0.0;
  philj = d0[itype][jtype] * (dexp*dexp - 2.0*dexp) - offset[itype][jtype];
  if (rsq < cut_ljsq[itype][jtype]) {
...
void *PairMorseBuckLJCoulDSF::extract(const char *str, int &dim)
{
  dim = 2;
